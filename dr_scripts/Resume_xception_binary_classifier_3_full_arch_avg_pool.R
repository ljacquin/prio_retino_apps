...
model<-load_model_hdf5('../DR_APPS/PRIO_RETINO_MODELS/xception_binary_classifier_3_full_arch_avg_pool.h5')

#=================================================#
# Augmenting train data with image data generator #
#=================================================#
train_dir='../DR_DATA/CNN_BINARY_DATA_3/train_dir/'
...
#===========================================================================#
valid_dir='../DR_DATA/CNN_BINARY_DATA_3/valid_dir/'
...
#============================#
train_dir_folders<-list.files('../DR_DATA/CNN_BINARY_DATA_3/train_dir/')
Vect_nb_elem_per_class<-rep(0,length(train_dir_folders))
for ( i in 1:length(train_dir_folders) )
{
  Vect_nb_elem_per_class[i]<-length(list.files(paste0('../DR_DATA/CNN_BINARY_DATA_3/train_dir/',train_dir_folders[i])))
}
...
history <- model %>% fit_generator(
  train_generator,
  steps_per_epoch=floor(length(list.files('../DR_DATA/CNN_BINARY_DATA_3/train_dir/',recursive=TRUE))/batch_size_val),
  epochs=num_epochs,
  validation_data=validation_generator,
  validation_steps=floor(length(list.files('../DR_DATA/CNN_BINARY_DATA_3/valid_dir/',recursive=TRUE))/batch_size_val),
...
save_model_hdf5(model, filepath=paste0('../DR_APPS/PRIO_RETINO_MODELS/xception_binary_classifier_3_full_arch_avg_pool.h5'))
write.table(img_size_cnn, file='../DR_APPS/PRIO_RETINO_MODELS/img_size_cnn.txt', col.names = FALSE, row.names = FALSE, quote = FALSE)
