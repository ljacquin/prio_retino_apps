...
length(non_quality_img)
length(non_quality_img)/nrow(df_img_score)

list_test_img_prio_retino <- list.files('../../DR_DATA/CNN_BINARY_DATA_1/test_dir/')
id_test_img <- match(list_test_img_prio_retino, df_img_score$image)
df_img_score_test <- df_img_score[id_test_img, ]
df_img_score_test_non_quality = df_img_score_test[df_img_score_test$quality==0,]

writeLines(df_img_score_test_non_quality$image, paste0('test_img_non_quality_',score_thresh))
