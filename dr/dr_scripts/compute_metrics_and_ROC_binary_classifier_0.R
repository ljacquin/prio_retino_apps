...
img_size <- 299
img_path <- "../dr_data/cnn_binary_data_0/test_dir/all_test_data/"
fundus_data_path <- "../dr_data/cnn_binary_data_0/test_dir/test_fundus/"
others_data_path <- "../dr_data/cnn_binary_data_0/test_dir/test_others/"
model <- load_model_hdf5("../dr_models/xception_binary_classifier_0_full_arch_avg_pool_epochs_3.h5")

Test_image_labels <- rbind(
  data.frame("level" = "fundus", "image_id" = list.files(fundus_data_path)),
  data.frame("level" = "others", "image_id" = list.files(others_data_path))
)
...
for (i in 1:nrow(Test_image_labels))
{
  ...
  ifelse((Vect_pos_class_prob[i] > 0.8), Vect_pred_class[i] <- "others", Vect_pred_class[i] <- "fundus")
}

df_class_pred_prob <- data.frame("True_class" = as.factor(Vect_true_class), 
                                 "Pred_class" = as.factor(Vect_pred_class), 
