...
img_size <- 299
img_path <- "../dr_data/cnn_binary_data_4/test_dir/all_test_data/"
maculo_data_path <- "../dr_data/cnn_binary_data_4/test_dir/test_maculopathy/"
rDR_data_path <- "../dr_data/cnn_binary_data_4/test_dir/test_rDR/"
model <- load_model_hdf5("../dr_models/xception_binary_classifier_4_full_arch_avg_pool_epochs_5.h5")

Test_image_labels <- rbind(
  data.frame("level" = "maculo", "image_id" = list.files(maculo_data_path)),
  data.frame("level" = "rDR", "image_id" = list.files(rDR_data_path))
)
...
for (i in 1:nrow(Test_image_labels))
{
  ...
  ifelse((Vect_pos_class_prob[i] > 0.5), Vect_pred_class[i] <- "maculo", Vect_pred_class[i] <- "rDR")
}
