      ...
      if (!is.null(unlist(rv$file1)) && as.numeric(rv$file1$size) > 1) {
        if ((!is.null(unlist(rv$patient_id))) && (unlist(rv$patient_id) != "")) {
          ...
          if (pred_disease_status == 1) {
            ...
            list_out_prio_retino$grad_cam_transformed_target_image <- magick::image_read(compute_grad_cam(
    ...
    output$outputImage1 <- renderImage(
      {
        ...
        if (!test_render_img) {
          ...
          list(
            src = tmpfile, contentType = "image/jpg",
    ...
    output$outputImage2 <- renderImage(
      {
        ...
        if (!test_render_img) {
          ...
          if (!is.null(list_out_prio_retino()$grad_cam_transformed_target_image)) {
