      ...
      if (!is.null(unlist(rv$file1)) && as.numeric(rv$file1$size) > 1) {
        if ((!is.null(unlist(rv$patient_id))) && (!is.null(unlist(rv$eye_side))) &&
          ...
          if (pred_disease_status == 1) {
            ...
            # compute grad classification activation mapping
            compute_grad_cam(cnn_model_path, img_path, img_size_cnn, last_conv_layer_name)
          }
          if (!img_quality) {
          ...
          HTML(paste0("<font color='#0c7683'>", i18n$t("Please reset Prio Retino first, then follow these instructions: 1. Select your language, 2. Insert patient identifier and 'left eye' or 'right eye'
 (without quotes), and 3. Upload a fundus image.  Do not change the selected language or patient references during an analysis, you might be charged further otherwise."), "</font>"))
        }
      } else {
        ...
        if (file.exists("www/grad_cam_transformed_target_image.jpg")) {
          file.remove("www/grad_cam_transformed_target_image.jpg")
        }
        paste0("")
      }
    })
    ...
    output$outputImage2 <- renderImage(
      {
        ...
        if (!test_render_img) {
          ...
          if (file.exists("www/grad_cam_transformed_target_image.jpg")) {
            list(
              src = "www/grad_cam_transformed_target_image.jpg", contentType = "image/jpg",
