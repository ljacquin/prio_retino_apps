      ...
      div(
        ...
        sidebarLayout(
          ...
          mainPanel(
            ...
            withLoader(plotOutput("outputImage"),
        ...
        titlePanel(h6(i18n$t("- Referable diabetic retinopathy (DR): moderate or superior signs of DR according to ICDR. AAO recommendations: repeat examination within 6 or 3 months for moderate or superi
or signs of DR respectively"),
                      style = "color:#0c7683", align = "left"
        )),
    ...
    list_out_prio_retino <- reactive({
      ...
      if (!is.null(unlist(rv$file1)) && as.numeric(rv$file1$size) > 1) {
        if ((!is.null(unlist(rv$patient_id))) && (unlist(rv$patient_id) != "")) {
          ...
          # Measure CNN computation time
          begin_cnn_comp <- Sys.time()
          ...
          # Measure original scaling computation time
          begin_orig_scal_comp <- Sys.time()
          ...
          print(paste0('original scaling computation time: ',signif(Sys.time() - begin_orig_scal_comp,2)))
          
          
          # Measure dr grad cam scaling computation time
          begin_dr_grad_cam_scal_comp <- Sys.time()
          ...
          if (pred_dr_status == 0) {
            ...
            if (Maculo_status) {
              ...
              out_dr_img <- out_dr_img %>% image_annotate(i18n$t("Detected areas for maculopathy"),
                                                          font = "monospace", color = "none", size = 12
              )
            } else {
          ...
          print(paste0('DR grad cam scaling and append computation time: ',
                       signif(Sys.time() - begin_dr_grad_cam_scal_comp,2)))
          
          
          
          # Measure dr grad cam scaling computation time
          begin_glauco_grad_cam_scal_comp <- Sys.time()        
          ...
          print(paste0('Glauco grad cam scaling and append computation time: ',signif(Sys.time() - begin_glauco_grad_cam_scal_comp,2)))
          
          print(paste0('Complete CNN computation time: ',signif(Sys.time() - begin_cnn_comp,2)))
          
          # Measure quality computation time
          begin_qual_comp <- Sys.time()
          ...
          print(paste0('Quality computation time: ',signif(Sys.time() - begin_qual_comp,2)))
          ...
          out_dr_prio_retino_txt <- HTML(paste0("<font color='#0c7683'>", i18n$t("Please reset Prio Retino first, then follow these instructions: 1. Select your language, 2. Insert patient identifier and 
3. Upload a fundus image."), "</font>"))
          out_glauco_prio_retino_txt <- HTML(paste0("<font color='#0c7683'>", i18n$t("Please reset Prio Retino first, then follow these instructions: 1. Select your language, 2. Insert patient identifier 
and 3. Upload a fundus image."), "</font>"))
        }
    ...
    output$outputImage <- renderPlot(
      {
        ...
        if (!test_render_img) {
          image_ggplot(image_read('www/AI_PRIO_RETINO.png'))
          # Default image
          # list(
          ...
          if (input$element_id == "Diabetic retinopathy and maculopathy" ||
              input$element_id == "Rétinopathie et maculopathie diabétique" ||
              input$element_id == "Retinopatia diabética e maculopatia") {
            # out_orig_dr_img_  <<- list_out_prio_retino$out_orig_dr_img
            # img_write = image_write(out_orig_dr_img_,
            #                         tempfile(fileext = "png"),
            #                         format = "png"
            # )
            #   
            # print(paste0('Plot computation time: ',signif(Sys.time() - begin_plot,2)))
            # list(
            #   src = img_write,
            #   contentType = "image/png",
            #   width = 1200, height = 550
            # )
            image_ggplot(list_out_prio_retino$out_orig_dr_img)
            
            # Return images for glaucoma
          } else {
            # out_orig_glauco_img_  <<- list_out_prio_retino$out_orig_glauco_img
            # img_write = image_write(out_orig_glauco_img_,
            #                         tempfile(fileext = "png"),
            #                         format = "png"
            # )
            # print(paste0('Plot computation time: ',signif(Sys.time() - begin_plot,2)))      
            ...
            #   width = 1200, height = 550
            # )
            image_ggplot(list_out_prio_retino$out_orig_glauco_img)
