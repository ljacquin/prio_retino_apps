...
server <- shinyServer(
  function(input, output, session) {
    options(shiny.maxRequestSize=0.8*1024^2)
    ...
    output$outputText <- renderText({
      if (!is.null(unlist(rv$file1))) {
        ...
        model = cnn_binary_classifier_1
        resized_img = "www/resized_cropped_target_image.jpg"
        
        #grad cam function
        grad_cam = function(pred_disease_status, model, x_target, resized_img) {
          test_image_output <- model$output
          last_conv_layer <- model %>% get_layer("xception", index = -1) #last layer
          grads <- k_gradients(test_image_output, last_conv_layer$output)[[1]]
          pooled_grads <- k_mean(grads, axis = c(1, 2, 3))
          iterate <- k_function(list(model$input),
                                list(pooled_grads, last_conv_layer$output[1,,,]))
          c(pooled_grads_value, conv_layer_output_value) %<-% iterate(list(x_target))
          for (i in 1:512) {
          ...
          write_heatmap <- function(heatmap, filename, 
                                    width = img_size_cnn, height = img_size_cnn,
          ...
          write_heatmap(heatmap, paste("img_heatmap.png", sep = ""))
          
          info <- image_info(resized_img)
          geometry <- sprintf("%dx%d!", info$width, info$height)
          pal <- col2rgb(viridis(20), alpha = TRUE)
          alpha <- floor(seq(0, 255, length = ncol(pal))) * 0.8
          pal_col <- rgb(t(pal), alpha = alpha, maxColorValue = 255) 
          write_heatmap(heatmap, paste("img_overlay.png", sep = ""),
