...
library(devtools)
library(reticulate)
# conda_create('/home/laval/miniconda3/envs/prio_retino_env', python_version = '3.6')
# py_install("/opt/image-quality-1.2.7/", envname = 'prio_retino_env', pip = TRUE)
# py_config()
# py_available()
# py_module_available("cv2")
# py_module_available("imquality")
use_condaenv(condaenv = "/home/laval/miniconda3/envs/3dr_env")
cv2 <- import("cv2")
...
source_python("resize_transformed_image_explanation.py")
...
if (!exists("cnn_binary_classifier_1") && !exists("cnn_binary_classifier_2") && !exists("cnn_binary_classifier_3") && !exists("cnn_binary_classifier_4")
  ...
  cnn_binary_classifier_2 <<- load_model_hdf5("xception_binary_classifier_2_full_arch_avg_pool_ratio_2_1_epochs_9.h5")
  ...
  # for lime
  explainer_labels <<- c(
    "0" = "Non referable diabetic retinopathy",
...
image_prep <- function(x) {
  arrays <- lapply(x, function(path) {
    img <- image_load(path, target_size = c(img_size_cnn, img_size_cnn))
    ...
    output$outputText <- renderText({
      if (!is.null(unlist(rv$file1))) {
        ...
        if (pred_disease_status == 1) {
          ...
          ## compute referable retinopathy classification explanation
          explainer <- lime(
            "www/transformed_target_image.jpg",
            as_classifier(cnn_binary_classifier_1, unlist(explainer_labels)),
            image_prep
          )
          set.seed(123)
          explanation <- explain(
          ...
          exp_img <- plot_image_explanation(explanation_df)

          ggsave("www/transformed_target_image_explanation.jpg", exp_img)
          resize_transformed_image_explanation(
            name = "www/transformed_target_image_explanation.jpg",
        ...
        if (!img_quality) {
          ...
          disease_diagnostic <- paste0(
            i18n$t("Warning: low quality image detected, Prio Retino results might be unreliable. "),
            disease_diagnostic
          )
        }
        ...
        if (file.exists("www/transformed_target_image_explanation.jpg")) {
          file.remove("www/transformed_target_image_explanation.jpg")
        }
        if (file.exists("www/resized_transformed_target_image_explanation.jpg")) {
          file.remove("www/resized_transformed_target_image_explanation.jpg")
        }
        paste0("")
      }
    })
    ...
    output$outputImage2 <- renderImage(
      {
        if (is.null(unlist(rv$file1))) {
          ...
          if (file.exists("www/resized_transformed_target_image_explanation.jpg")) {
            list(
              src = "www/resized_transformed_target_image_explanation.jpg", contentType = "image/jpg",
